...
Aeq = [];
beq = [];
%objective function to minimize (weighted squared norm of joint velocities)
fun=@dq_cost_function;
nonlcon=@balistic_eq;
options=optimoptions('fmincon','Algorithm','active-set','Display','iter-detailed');
x = fmincon(fun,x0,A,b,Aeq,beq,lb,ub,nonlcon,options);

%% plot the trajectory done by the ball with optimized value of p_ee and dp_ee
figure
axis([6 x_goal+0.5 -0.1 3]);
daspect([1 1 1]);
plot_robot(x);
...
%% Finding the throwing robot trajectory via optimization
%set sampling time and throwing time
global delta T_t;
delta=0.025;
T_t=1;
n=T_t/delta;
...
x_init=[6 9*pi/10 -pi/4 0 0 0]';
x_f=x;
des=zeros(9*n+6,1);
for k=1:6
    des(k:9:9*n+6)=linspace(x_init(k),x_f(k),n+1);
end
...
%% Step 2 Optimization
x_init=[9.3647;1.4062;pi/2;0;0;0];
A = [];
b = [];
Aeq = zeros(size(des,1));
beq = zeros(size(des,1),1);
Aeq(1:6,1:6)=eye(6); beq(1:6)=x_init; %constrain initial state
Aeq(9*n+1:9*n+6,9*n+1:9*n+6)=eye(6);
beq(9*n+1:9*n+6)=x_f;
...
options=optimoptions('fmincon','Algorithm','sqp','Display','iter-detailed', ...
                     'MaxFunctionEvaluations',10*1.86e04, 'MaxIterations',300);
[sol,cost] = fmincon(fun,des,A,b,Aeq,beq,lb,ub,nonlcon,options);
save("data/data_throw_1.mat","sol","cost","x_init","mball","db","x_goal","delta","T_t","q_max","q_min","w_max","t_max","options");
%% Stopping phase optimization
mball=0;
T_s=1.25;
n=T_s/delta;
%initialize the decision variables
%for the state simple linear interpolation between init and final state
%for the inputs set them to 0
x_throw=sol(end-5:end);
x_f=[x_throw(1)+1; pi/2; 0; 0; 0; 0];
des=zeros(9*n+6,1);
for k=1:6
    des(k:9:9*n+6)=linspace(x_throw(k),x_f(k),n+1);
end
...
Aeq = zeros(size(des,1));
beq = zeros(size(des,1),1);
% equality constraint: initial state of stopping phase = throwing state
Aeq(1:6,1:6)=eye(6);
beq(1:6)=x_throw;
% equality constraint: zeroing final joint velocities of stopping phase
Aeq(end-2:end,end-2:end)=eye(3);
nonlcon=@stopping_constraints;
options=optimoptions('fmincon','Algorithm','sqp','Display','iter-detailed', ...
                     'MaxFunctionEvaluations',10*1.86e04, 'MaxIterations',500);

[stop_sol,stop_cost] = fmincon(fun,des,A,b,Aeq,beq,lb,ub,nonlcon,options);
save("data/data_stop_1.mat","stop_sol","stop_cost","x_init","mball","db","x_goal","delta","T_s","q_max","q_min","w_max","t_max","options");
