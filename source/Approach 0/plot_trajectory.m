...
file_number="1";
load("data/data_throw_" + file_number + ".mat");
...
for k=1:9:loops*9
    ...
    x=dt_dynamics(f,x,u,delta);
    cnt=cnt+1;
end
if (~fall)
    lower_x=sol(1)-0.8;
    load("data/data_stop_" + file_number + ".mat");
    factor=delta*1000;
    fall=false;
    loops=(size(stop_sol,1)-6)/9;
    f = @ct_dynamics;
    for k=1:9:loops*9
        fall=false;
        u=stop_sol(k+6:k+8);
