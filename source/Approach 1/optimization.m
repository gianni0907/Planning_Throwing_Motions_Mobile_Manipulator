...
%% Setting parameters for throwing phase optimization
global delta;
delta=0.025;
T_t=1;
n=T_t/delta;
...
%% Step 1 Optimization (throwing phase)
A = [];
b = [];
...
%% Setting parameters for stopping phase optimization
mball=0;
T_s=1.25;
n=T_s/delta;
%initialize the decision variables
...
%% Step 2 Optimization (stopping phase)
A = [];
b = [];
Aeq = zeros(size(des,1));
beq = zeros(size(des,1),1);
