...
length_ee=0.07;
end_effector=[p3(1)-length_ee*(cos(x(2)+x(3))) p3(1)+length_ee*(cos(x(2)+x(3)));
...
base=T*R*(iT*base);
end_effector=T*R*(iT*end_effector);
...
for k=1:4
    ...
    elseif k==4
        ...
        plot(end_effector(1,1:2),end_effector(2,1:2),'Color','b','LineWidth',3)
    else
        plot(p(1,k),p(2,k),'.','MarkerSize',20,'Color','b')
    end
end
